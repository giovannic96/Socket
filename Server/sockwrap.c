...
/* reads exactly "n" bytes from a descriptor */
ssize_t protocol_readn (int fd, void *vptr, size_t n, int timeout_sec)
{
	/* Select parameters */
	fd_set cset;
	...
	nleft = n;
	while (nleft > 0) {
		if((ns = select(fd+1, &cset, NULL, NULL, &timeout)) == -1)
			return 0;	
		if(ns > 0) {				
			if(FD_ISSET(fd, &cset)) {
				if ( (nread = read(fd, ptr, nleft)) < 0) {
...
ssize_t protocol_readline_unbuffered (int fd, void *vptr, size_t maxlen, int timeout_sec)
{
	/* Select parameters */
	fd_set cset;
  	struct timeval timeout;
		...
		if(ns > 0) {		
			if(FD_ISSET(fd, &cset)) {
				if ( (rc = recv(fd,&c,1,0)) == 1) {
					*ptr++ = c;
					if(c == '\r') {
						posR = n; //get position of '\r'
						foundR = 1;
					} else if ((n - posR) == 1 && foundR == 1)
						if(c == '\n') //if last element is '\n'
							break;  
						else 
							foundR = 0;
