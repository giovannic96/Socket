...
int main(int argc, char* argv[]) {
	...
	while(1) {		
		...
		if(signal(SIGPIPE, SIG_IGN) == SIG_ERR) //to prevent errors (zombies) when ctrl-c occurs while data are transfering in the socket
			exit(-1);
		
		if(signal(SIGCHLD, zombieHandler) == SIG_ERR) //to prevent zombie creation
			exit(-1);
		...
		if((childpid = fork()) < 0) {
			err_msg("Errore nella creazione del processo figlio");
			continue; //restart from first while(1)
